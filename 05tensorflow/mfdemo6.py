...
x = [[2],[2]];
myval = tf.reduce_sum(x)#3
myval = tf.square(x)#[[1] [4]]
myval = tf.reduce_mean(x)#1
myval = tf.reduce_mean(tf.reduce_sum(tf.square(x), reduction_indices=[1]))#2
...
with tf.Session() as sess:
    ...
    for i in range(1000):
        ...
        if i % 50 == 0:
            ...
            # print(sess.run(myval,feed_dict=feed_dict))
